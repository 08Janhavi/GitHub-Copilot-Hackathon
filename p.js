...
// Create an object to store the amounts for each description and category
const amountsByDescription = transactions.reduce((acc, transaction) => {
  const { description, amount, category } = transaction;
  if (!acc[description]) {
    acc[description] = { amount: 0, category };
  }
  acc[description].amount += Math.abs(amount);
  return acc;
}, {});

// Calculate the total balance
const totalBalance = Object.values(amountsByDescription).reduce((acc, { amount }) => acc + amount, 0);

// Calculate the percentages for each description and category
const percentagesByDescription = {};
for (const description in amountsByDescription) {
  const { amount, category } = amountsByDescription[description];
  const percentage = ((amount / totalBalance) * 100).toFixed(2);
  percentagesByDescription[description] = { percentage, category };
}

// Prepare the data for the chart
const labels = Object.keys(percentagesByDescription);
const percentagesData = Object.values(percentagesByDescription).map(({ percentage }) => percentage);
const colors = Object.values(percentagesByDescription).map(({ category }) =>
  category.toLowerCase() === 'expense' ? 'rgba(255, 0, 0, 0.8)' : 'rgba(0, 128, 0, 0.8)'
);
    ...
    tooltips: {
      callbacks: {
        label: function (tooltipItem, data) {
          ...
          const category = percentagesByDescription[label].category;
          return `${label} (${category}): ${percentage}%`;
        },
      },
    },
  },
